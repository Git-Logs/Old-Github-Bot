...
module.exports = {
    ...
    category: 'Git',
    aliases: ['unsubscribe'],
    description: 'UnSubscribe from Updates for a GitHub Repo in the current Channel.',
    args: true,
    usage: 'ghu!unsub <Repo Link>',
    permissions: [],
    ownerOnly: false,
    ...
    execute: async (message, args, client) => {
        ...
        let repoExists = new MessageEmbed()
          .setTitle('🚫 Error: Unknown_GitHub_Repo')
          .setColor(client.embedColor)
          .setDescription('Woah, This channel is not subbed to updates from that Repo!')
        ...
        if (!FindRepo) return message.channel.send({embeds: [repoExists]})
            ...
            await GitHubRepositories_DB.deleteOne({
                channel: message.channel.id,
                repo: repo
            });

            let unsubSuccess = new MessageEmbed()
             .setTitle('✔️ Removed Subscription!')
             .setColor(client.embedColor)
             .setDescription(`Deleted a Subscription for: ${message.channel.name}!`)
             .addField('Subscription', `${repo}`, true)
